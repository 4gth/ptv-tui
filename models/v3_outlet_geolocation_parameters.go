// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"

	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
)

// V3OutletGeolocationParameters v3 outlet geolocation parameters
//
// swagger:model V3.OutletGeolocationParameters
type V3OutletGeolocationParameters struct {

	// Filter by maximum distance (in metres) from location specified via latitude and longitude parameters (default = 300)
	MaxDistance float64 `json:"max_distance,omitempty"`

	// Maximum number of results returned (default = 30)
	MaxResults int32 `json:"max_results,omitempty"`
}

// Validate validates this v3 outlet geolocation parameters
func (m *V3OutletGeolocationParameters) Validate(formats strfmt.Registry) error {
	return nil
}

// ContextValidate validates this v3 outlet geolocation parameters based on context it is used
func (m *V3OutletGeolocationParameters) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *V3OutletGeolocationParameters) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *V3OutletGeolocationParameters) UnmarshalBinary(b []byte) error {
	var res V3OutletGeolocationParameters
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
